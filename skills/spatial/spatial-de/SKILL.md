...
- **Paired design auto-activates when samples overlap groups.** If the same `sample_id` value appears in both `group1` and `group2`, the wrapper switches the DESeq2 formula to `~ sample_id + condition` (`spatial_de.py:1211`).  Check `result.json["summary"]["paired_design"]` (also surfaced at `spatial_de.py:454`); if the run was unintentionally paired (e.g. the user merged samples by accident), the LFC interpretation changes.
...
- `references/r_visualization.md` — R customization layer reading `figure_data/`
- Adjacent skills: `spatial-preprocess` (upstream prerequisite), `spatial-domains` (upstream cluster discovery), `spatial-genes` (sibling: spatially variable gene discovery, not group-DE), `spatial-condition` (sibling: condition comparison without per-cluster context), `sc-de` / `bulkrna-de` (same-question DE for the other two data modalities)
