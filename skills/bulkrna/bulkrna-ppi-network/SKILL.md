...
| Output | Path | Notes |
|---|---|---|
| Edge list | `tables/string_edges.csv` | source / target / score |
| Hub genes | `tables/hub_genes.csv` | top-N by degree centrality |
...
- **STRING-API-failure fallback is to DEMO edges, not a smaller real query.**  `bulkrna_ppi_network.py:118` catches any API exception and substitutes the built-in toy network.  This means a transient network outage produces output that looks normal but is unrelated to the user's gene list.  Always check `result.json["edges_source"]` — `"string_api"` for real, `"builtin_demo"` for the silent fallback.
