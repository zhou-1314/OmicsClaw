...
- **`keyword` mode depends on `trigger_keywords` in `skill.yaml`.** Stale or missing keywords degrade routing quality silently. The 96 v2 skills' keyword tables are loaded on every call via the registry.
