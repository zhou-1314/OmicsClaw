...
├── member.py                  ConsensusMember 数据类
├── dispatch.py                TYPED_CONSENSUS_REGISTRY + select_consensus_mode + consensus_namespace + output_banner
...
└── narrative/
    ...
    ├── synthesizer.py         ★ N JSONs → 综合 markdown（contradictions 强制段；banner 不可关）
    └── prompts/
        ├── extract.tmpl
        └── synthesize.tmpl
```
...
- **Consensus member** —— `(skill, params, artifact_relpath, label_column, intrinsic_quality_path)` 元组；**一个 deterministic skill subprocess**，不是 LLM sub-agent
- **Evaluation chair** —— LLM 评审主席。只挑成员 + 解读结果，**不做统计合并**（mode-voting 由 operator 负责）
- **Base clusterings (BC)** —— 用户在 fan-out 后从分数排名里挑出的子集（CLI 交互；Desktop/Channel 走 top-K 默认）
- **Typed Consensus Registry** —— `dispatch.py` 单文件 allowlist；新 skill 加入必须显式注册 + ADR review
...
| **plan / narrative 的 LLM 调用** | 复用 `omicsclaw.routing.llm_router._resolve_llm_config`（OpenAI-兼容 endpoint） |
...
├── test_team_runtime.py          25 cases — 并行 fan-out + cancel + timeout 不广播
...
共 100 passed + 2 skipped（R / 网络 gate）。
