...
RET-04 的 `.h5ad` 探针、三态评估和 `skill=auto` 执行前硬门。M1 的任意 Python/复杂 lineage 泛化仍有限，
M2 仍缺 candidate DAG/topo chain，M3 仍只有重复成功提示原型。
...
| M2 检索 | **partial（retrieval truth + precondition 安全纵切）** | RET-01/02/03 verified；RET-04 partial；RET-05 missing；RET-06 partial | 29-case/8-domain oracle 接入 CI；resolver 消费 lifecycle/validation/skip；`.h5ad` 探针、三态评估、AnalysisRouter preflight 和 auto-run 硬门已落地；显式 skill 入口统一门、candidate-wide penalty、更多格式与 candidate DAG 仍缺 |
...
| H4：检索仍是 domain-first sparse，结构条件未进入决策 | **部分关闭，RET-03/04/06 已有纵切** | resolver 现过滤 draft/deprecated、使用 validation tie-break、消费 structured `skip_when`，并对选中技能携带三态 precondition/`execution_ready`；compatibility graph 与候选级 penalty 仍未实现 |
...
路由预算 ceiling 同样已版本化并进 CI。RET-04 的安全纵切已经实现，但 candidate-wide
penalty、更多文件格式探针与 `sc_batch` 收敛尚未完成；下一主线是 RET-05 候选兼容图/
复合 topo chain，不要先上 dense retrieval 掩盖结构契约覆盖不足。
...
上述 1–2 已完成、3 与 RET-04 有可验收纵切；4–5、RET-04 剩余项与 RET-05 完成并通过矩阵后，才可
宣称达到原始四阶段目标。

### 8.6 RET-04 前置条件安全纵切复验
...
- 边界：硬门目前只在 `skill=auto` 自动执行入口强制，显式 skill 仍待统一；caller-supplied
  profile 仅作规划，config 仍需 surface 显式注入；当前也不是 candidate DAG 或所有候选重排，
  非 h5ad 只探测文件类型。因此 RET-04 记为 partial，不宣称结构检索闭环完成。
