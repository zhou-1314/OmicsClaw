...
trace-provable call workflow 的 facade-free 泛化与 8 域 routing oracle。M1 的任意
Python/复杂 lineage 泛化仍有限，M2 仍缺 precondition/DAG，M3 仍只有重复成功提示原型。
...
| M2 检索 | **partial（retrieval truth 已建立）** | RET-01/02/03 verified；RET-04/05 missing；RET-06 partial | 26-case/8-domain oracle 全局和逐域门接入 CI；resolver 消费 lifecycle/validation/skip；precondition penalty 与 candidate DAG 仍缺 |
...
| H4：检索仍是 domain-first sparse，结构条件未进入决策 | **确认，RET-03/06 已首轮修复** | resolver 现过滤 draft/deprecated、使用 validation 小权重 tie-break，并消费 structured `skip_when`；precondition/compatibility graph 仍未实现 |
...
`tests/fixtures/routing_oracle/v1.json`：26 cases、8 domains、每域至少 3 条；评估器输出
precision@1、top-3 recall、domain/decision accuracy 和 hallucinated alias rate，并执行
逐域门。当前 v1 全局及 8 域指标均 1.000（alias rate 0.000），CLI 已接入 PR CI。
该结果只对版本化语料成立，不等价于线上真实请求 100% 准确。

路由预算 ceiling 同样已版本化并进 CI。下一步是 RET-04 precondition penalty 和 RET-05
候选兼容图/复合 topo chain；不要先上 dense retrieval 掩盖结构契约覆盖不足。
...
上述 1–2 已完成、3 有可验收纵切；4–5 与 RET-04/05 完成并通过矩阵后，才可
宣称达到原始四阶段目标。
...
### 8.7 Acquisition 泛化与全域 oracle 追加证据
...
- routing oracle：26 cases / 8 domains；precision@1=1.000、top-3 recall=1.000、
