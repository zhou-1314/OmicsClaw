...
接到显式 skill/shared runner。M1 的任意 Python/复杂 lineage 泛化仍有限，
...
| M2 检索 | **partial（retrieval truth + precondition 安全纵切）** | RET-01/02/03 verified；RET-04/04b verified 子集；RET-05 missing；RET-06 partial | 29-case/8-domain oracle 接入 CI；resolver 消费 lifecycle/validation/skip；`.h5ad` 探针、三态评估、auto-run 与显式 shared-runner 硬门已落地；candidate-wide penalty、更多内容探针与 candidate DAG 仍缺 |
...
路由预算 ceiling 同样已版本化并进 CI。RET-04/04b 的 auto + explicit 安全纵切已经实现，但 candidate-wide
...
上述 1–2 已完成、3 与 RET-04/04b 有可验收纵切；4–5、RET-04 剩余内容探针与 RET-05 完成并通过矩阵后，才可
宣称达到原始四阶段目标。
