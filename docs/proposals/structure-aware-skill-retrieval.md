...
> Gate Seam；候选级 penalty/compatibility graph 与 topo chain 仍未落地。当前验收状态以
...
>
> 状态：实施草案 v0.4（RET-04/04b 执行安全纵切已落地，RET-05 compatibility DAG 实施中；v0.1 经 Codex/gpt-5.5(xhigh) 独立审阅，已纳入其"必须修正"6 项 + "建议优化"5 项；全记录见附录 C）
...
- RET-04b 将 `preflight_skill_execution()` 接到 shared runner 的 `_prepare_skill_run()`：显式
  skill、alias、session、sync/async、CLI/agent/desktop/jobs 均在输出目录创建与 subprocess 之前
  经过同一 Gate Seam。无输入/空 session、缺失/损坏 `.h5ad` fail closed；demo 保持原合同。
- `interface.inputs.path_kinds` 形式化 `file | directory | freeform` 并由 SKILL.md generator 消费；
...
- 尚未完成：非 `.h5ad` 的结构化内容探针、把 `sc_batch` 迁为通用 evaluator 消费者、
  candidate-wide penalty/compatibility graph；因此 RET-04b 完成不等于 RET-05/完整 retrieval 完成。
