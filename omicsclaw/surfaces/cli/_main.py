...
This is the CLI body. It is reached three ways, all equivalent:

    omicsclaw <args>            # console script (pip / npm install)
    oc <args>                   # short alias for the same entrypoint
    python omicsclaw.py <args>  # repo-root shim, source checkouts only

The examples below use the bare `omicsclaw` form; substitute either of the
other two freely.

Usage:
    omicsclaw list
    omicsclaw run <skill> --demo
    omicsclaw run <skill> --input <data> --output <dir>
    omicsclaw run spatial-pipeline --input <h5ad> --output <dir>
    omicsclaw upload --input <data> --data-type <type>

Interactive CLI/TUI:
    omicsclaw interactive               # Rich CLI (prompt_toolkit)
    omicsclaw interactive --ui tui      # Full-screen Textual TUI
    omicsclaw interactive -p "..."      # Single-shot mode
    omicsclaw interactive --session <id> # Resume session
    omicsclaw tui                       # Alias for --ui tui

MCP Server Management:
    omicsclaw mcp list
    omicsclaw mcp add <name> <command> [args]
    omicsclaw mcp remove <name>
    omicsclaw mcp config
"""
...
# Stdlib-only import surface, so it is safe to resolve the workspace before the
# heavier package imports further down.
...
# Resolved rather than derived from ``__file__``: this module ships inside the
# ``omicsclaw`` package, so its own location is site-packages under a pip / npm
# install and says nothing about where the user's workspace is. The shared
# resolver answers that question for every install shape — see
# ``omicsclaw.common.workspace.resolve_omicsclaw_dir``.
OMICSCLAW_DIR = resolve_omicsclaw_dir()
...
# NOTE: this module used to prepend OMICSCLAW_DIR to sys.path here, back when it
# was the repo-root ``omicsclaw.py`` and needed the repo root importable to
# reach the ``omicsclaw`` package. It now ships inside that package, so the
# imports below resolve on their own — and OMICSCLAW_DIR may legitimately be
# ``~/.omicsclaw`` under a pip / npm install, which must not join sys.path.
# The workspace IS still added to sys.path where that is actually wanted —
# inside `upload_session()`, which needs a user checkout's ``bot.*`` importable.
