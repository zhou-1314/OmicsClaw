...
_HARDCODED_SKILLS: dict[str, dict[str, Any]] = {
    ...
    "sc-cell-annotation": {
        ...
        "demo_args": ["--demo"],
        "description": "Cell type annotation (markers, CellTypist, PopV, KNNPredict, SingleR, scmap, SCSA)",
        "allowed_extra_flags": {
            "--method", "--reference", "--cluster-key", "--model",
            "--celltypist-majority-voting", "--no-celltypist-majority-voting",
            "--marker-file", "--manual-map", "--manual-map-file",
            "--species", "--tissue", "--scsa-foldchange", "--scsa-pvalue",
        },
        "saves_h5ad": True,
    },
    ...
    "sc-pseudotime": {
        ...
        "demo_args": ["--demo"],
        "description": "Pseudotime and lineage inference with DPT, Palantir, VIA, CellRank, or Slingshot",
        "allowed_extra_flags": {
            "--cluster-key", "--use-rep", "--root-cluster", "--root-cell", "--end-clusters",
            "--n-neighbors", "--n-pcs", "--n-dcs",
            "--n-genes", "--method", "--palantir-knn", "--palantir-n-components", "--palantir-num-waypoints",
            "--palantir-max-iterations", "--palantir-seed", "--via-knn", "--via-seed",
            "--cellrank-n-states", "--cellrank-schur-components", "--cellrank-frac-to-keep",
            "--cellrank-use-velocity", "--corr-method",
        },
        "requires_preprocessed": True,
        "saves_h5ad": True,
    },
    ...
    "sc-batch-integration": {
        ...
        "demo_args": ["--demo"],
        "description": "Multi-sample integration and batch correction (Harmony, scVI, BBKNN, Scanorama, SIMBA, fastMNN, Seurat CCA/RPCA)",
        "allowed_extra_flags": {"--method", "--batch-key", "--n-epochs", "--no-gpu", "--n-latent", "--labels-key", "--harmony-theta", "--bbknn-neighbors-within-batch", "--scanorama-knn", "--integration-features", "--integration-pcs", "--simba-n-top-genes", "--simba-n-components", "--simba-k", "--simba-num-workers"},
        "saves_h5ad": True,
    },
    "sc-de": {
        "domain": "singlecell",
    ...
    "sc-markers": {
        ...
        "demo_args": ["--demo"],
        "description": "Find marker genes for cell clusters using Wilcoxon, t-test, logistic regression, or COSG",
        "allowed_extra_flags": {
    ...
    "sc-pathway-scoring": {
        "domain": "singlecell",
        "alias": "sc-pathway-scoring",
        "script": SKILLS_DIR / "singlecell" / "scrna" / "sc-pathway-scoring" / "sc_pathway_scoring.py",
        "demo_args": ["--demo"],
        "description": "Single-cell pathway and gene-set activity scoring (AUCell R, AUCell Python, score_genes)",
        "allowed_extra_flags": {
            "--method", "--gene-sets", "--gene-set-db", "--species", "--groupby",
            "--top-pathways", "--aucell-auc-max-rank",
            "--score-genes-ctrl-size", "--score-genes-n-bins",
    ...
    "sc-cytotrace": {
        "domain": "singlecell",
        "alias": "sc-cytotrace",
        "script": SKILLS_DIR / "singlecell" / "scrna" / "sc-cytotrace" / "sc_cytotrace.py",
        "demo_args": ["--demo"],
        "description": "Cell potency prediction using gene expression complexity (CytoTRACE-style)",
        "allowed_extra_flags": {"--method", "--n-neighbors"},
        "requires_preprocessed": True,
        "saves_h5ad": True,
    },
