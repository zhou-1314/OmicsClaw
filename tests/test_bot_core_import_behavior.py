...
def test_bot_core_lazy_TOOLS_attribute_resolves_without_nameerror():
    """``omicsclaw.runtime.agent.state.TOOLS`` is a legacy lazy re-export of the tools-list
    produced by ``omicsclaw.runtime.agent.loop.get_tools()``. The ``__getattr__`` branch
    ...
    every access to ``omicsclaw.runtime.agent.state.TOOLS`` raises NameError.

    Sibling lazy attributes ``TOOL_RUNTIME`` and ``TOOL_EXECUTORS`` work
    because their backing functions are imported eagerly from
    ``omicsclaw.runtime.tools.builders.agent_executors`` at the top of the file.
    ...
    assert isinstance(tools, list), (
        f"omicsclaw.runtime.agent.state.TOOLS should be a list of tool definitions; "
    ...
    assert any(
        ...
        f"omicsclaw.runtime.agent.state.TOOLS missing the 'omicsclaw' tool — get_tools() likely "
