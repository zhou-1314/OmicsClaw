...
def test_message_bus_module_deleted() -> None:
    ...
    assert find_spec("omicsclaw.surfaces.channels.bus") is None


def test_middleware_module_deleted() -> None:
    """``bot/channels/middleware.py`` was deleted in Phase 1 P0-A Slice C."""
    assert find_spec("omicsclaw.surfaces.channels.middleware") is None


def test_run_py_does_not_build_middleware() -> None:
    """``omicsclaw/surfaces/channels/__main__.py`` and
    ``omicsclaw/surfaces/desktop/server.py`` must not reference
    ``_build_middleware`` (deleted in Phase 1 P0-A Slice A)."""
    offenders: list[str] = []
    for rel in ("omicsclaw/surfaces/channels/__main__.py", "omicsclaw/surfaces/desktop/server.py"):
        path = REPO_ROOT / rel
        if not path.exists():
            continue
