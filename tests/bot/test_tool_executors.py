...
@pytest.mark.asyncio
async def test_autonomous_analysis_rejects_python_script_inputs_before_engine_start(
    tmp_path,
    monkeypatch,
):
    import omicsclaw.runtime.agent.state  # noqa: F401 - initializes re-export cycle
    ...
    async def fake_loop(request, **kwargs):
        called["engine"] = True
        raise AssertionError("engine started for an unsupported script input")
    ...
    result = await execute_autonomous_analysis_execute(
        {
            "goal": "run the existing script",
            "input_paths": [str(script)],
        }
    )

    assert called["engine"] is False
    assert "does not execute prewritten Python scripts" in result
    assert "complete analysis objective" in result
