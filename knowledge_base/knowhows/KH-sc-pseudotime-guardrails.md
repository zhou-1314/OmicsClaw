...
search_terms: [pseudotime, DPT, PAGA, diffusion map, Palantir, VIA, root cluster, trajectory genes, 单细胞拟时序, 轨迹, 调参]
priority: 1.0
source_urls:
...
- **Use method-correct language**: in the current wrapper, `method` selects the trajectory algorithm (`dpt`, `palantir`, or `via`), while `corr_method` only controls how trajectory-associated genes are ranked afterward.
